                ...
                <div class="questions-page-questions">
                    <div class="questions-page-question-wrapper">
                        <div class="questions-page-question-details">
                            <span class="questions-page-question-details-number"><?= $assignmentDetails[0]->name ?></span>
                            <span class="questions-page-question-details-mark"><?= $assignmentDetails[0]->mark_value ?> mark</span>
                        </div>
                        <div class="questions-page-question-container">
                            <img src="" alt="">
                            <span class="questions-page-question"><?= $assignmentDetails[0]->description ?></span>
                            <div class="course-quiz-list-quiz">
                                ...
                                <?php $filename = getFileNameFromPath($student_attempt[0]->file) ?>
                                <a  href="<?= ROOT ?>StudentAssignments/download/<?= $filename ?>/<?= displayAssignmentSubmision($student_attempt) ?>" class="course-quiz-list-link"><?= displayAssignmentSubmision($student_attempt) ?></a>
                            </div>
                                
                            <div class="form-group" style='color:blue ;font-weight: bold; font-size: 1.6rem;'>
                                <label for="mark1">Mark</label>
                                <input id ='mark1' required value="<?= $student_attempt[0]->mark ?>" name="<?= $student_attempt[0]->id ?>" type="number" class="form-control" placeholder="Mark" style="font-size: 1rem;">
                            </div>
                            <?php if (isset($errors) && isset($errors[$student_attempt[0]->id])): ?>
                                    <em for="<?= $student_attempt[0]->id ?>" class="invalid"><?= ucfirst($errors[$student_attempt[0]->id]) ?></em>
                            <?php endif; ?>   
                        </div>
                    </div>
                    <div class="questions-page-move">
                        <button class="questions-page-move-previous" type="button">cancel</button>
                        <button class="questions-page-move-next" type="submit">Submit</button>
                    </div>
                </div>
            </form>
            </div>
